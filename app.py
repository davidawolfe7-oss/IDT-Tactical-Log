...
# --- SCREEN 2: TACTICAL SORTIE (THE ONE-STOP SHOP) ---
else:
    ...
    with col_check1: use_pov = st.checkbox("POV Driven (Airport/Drill)", value=True)
...
st.markdown(f"""
    <div class="metric-card">
        ...
        <p style='color:#ccc;'>Includes POV Miles, Flights, Rentals, and Meals (50%) minus ${reimbursement} reimbursement.</p>
    </div>
    """, unsafe_allow_html=True)

if st.button("💾 LOCK MISSION LOG", use_container_width=True):
    # Only update the Odometer chain if POV was actually used
    ...
    conn.commit()
    st.markdown("<div class='success-banner'>MISSION SECURED. ALL LOGISTICS ARCHIVED.</div>", unsafe_allow_html=True)
